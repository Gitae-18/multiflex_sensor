...
#define SPI_BUS 0
#define SPI_DEVICE 0
#define SENSOR_DEVICE "/dev/ttyAMA2"
#define EN_PIN 4
...
int main(){
    ...
    if (serDataAvailable(serial_port) > 0) {
        printf("Enable to Serial\n");
        printf("%c\n", serReadByte(serial_port));
    }
	...
	double xr,yr,zr;
	while(1) {
		    sendData(serialPort, data, data_length);
	             time_sleep(1);
		    sendSerialData(serial_port, command, arrayLength);
	             time_sleep(1); // 1초 대기
...
/*	            int tempMSB = receivedData[11];
		...
		get_axes(&xr, &yr, &zr);
		double dataToSet[] = {temp, humi, pm25, pm10, pm1, co2, voc, ch2o, co, o3, no2, xr, yr, zr, warningT, alarmT};
		...
		snprintf(sensorDataString, sizeof(sensorDataString),
	            ...
	            "\"no2\": %.3f, \"x\": %.3f, \"y\": %.3f, \"z\": %.3f, \"wargningTemp\": %.1f, \"alarmTemp\": %.1f}",
	             temp, humi, pm25, pm10, pm1, co2, voc, ch2o, co, o3, no2, xr, yr, zr, warningT, alarmT);
		 ...
		 usleep(100);
   		 if(curl) {
				...
				curl_easy_setopt(curl, CURLOPT_URL, "http://192.168.0.36:4001/setdata");
